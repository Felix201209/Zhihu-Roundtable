...
知辩圆桌不是“又一个 AI 写作工具”。它把知乎热榜变成一条持续讨论链路：

1. **筛话题**：按争议度、资料量、站内讨论空间和下一轮内容潜力排序。
2. **过三问**：能站队？反方成立？证据够吗？
3. **收评论**：新反方、真实经验和高质量补充进入下一轮话题。

AI 不替用户表达观点。它把一次热点整理成有证据、有分歧、有后续的知乎讨论。
...
- 可选变量：`PUBLIC_APP_URL`、`ZHIHU_OAUTH_REDIRECT_URI`、`ZHIHU_OAUTH_APP_ID`、`ZHIHU_OAUTH_APP_KEY`、`ZHIHU_OAUTH_CLIENT_ID`、`ZHIHU_OAUTH_AUTHORIZE_URL`、`ZHIHU_OAUTH_TOKEN_URL`、`ZHIHU_OAUTH_TOKEN_BODY_MODE`、`ZHIHU_OAUTH_USER_INFO_URL`、`ZHIHU_OAUTH_USER_FOLLOWERS_URL`、`ZHIHU_OAUTH_SCOPE`、`ZHIHU_OAUTH_CLIENT_SECRET`。`ZHIHU_OAUTH_APP_ID`/`ZHIHU_OAUTH_APP_KEY` 是官方命名别名，等价于 client id/secret。

AI 成本保护：

- `AI_USAGE_GUARD_MODE=off|ip|oauth|oauth_or_ip`。公网演示可先用 `ip` 做每日限额；官方 OAuth endpoint 配齐后切到 `oauth`，所有会消耗模型额度的接口都会要求知乎登录。
- `AI_USAGE_DAILY_CREDITS` 设置默认每日额度；可用 `AI_USAGE_ANON_DAILY_CREDITS` 和 `AI_USAGE_AUTH_DAILY_CREDITS` 分开控制匿名/IP 与 OAuth 用户。
- `GET /api/usage/status` 返回当前身份的已用、剩余、重置时间。
- `POST /api/usage/reward/project-share` 接收知乎站内项目介绍链接，按 `AI_USAGE_PROJECT_SHARE_REWARD_CREDITS` 追加一次性额度；同一身份同一链接不会重复领取。
- 被限额保护的接口包括模型探针、脑洞生成/报告、圆桌完整 run、SSE run、prepare/debate/publish-draft/feedback。真实发布/评论/reaction 仍保持用户确认 token，不会自动写入知乎。
