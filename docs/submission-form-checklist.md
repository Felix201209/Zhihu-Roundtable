...
`evidence:submission` 只读打印当前 commit、源码包 sha256、截图尺寸和最终外部验收命令，方便复制到提交备注或路演备忘；`node scripts/print-submission-evidence.mjs --markdown` 会输出更适合直接粘贴的 Markdown 版。正式证据要求工作区干净，本地预览才追加 `--allow-dirty`。
