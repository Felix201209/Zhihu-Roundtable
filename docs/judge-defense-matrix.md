...
| 会不会自动发帖或误触真实知乎写操作？ | 不会。发布、主持评论和 reaction 都需要一次性 confirmation token，服务层默认拒绝 live 写。 | 展示前端确认发布按钮，或说明 HTTP API 没 token 会返回 `confirmation_required`。 | `src/backend/http-server.ts`、`src/backend/workflow-service.ts`、`tests/http-server.test.ts` |
