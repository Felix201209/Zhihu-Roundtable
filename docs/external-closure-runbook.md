...
- 如果 GitHub API 临时 EOF，`verify:external-preflight` 会先重试 3 次，仍失败才输出 warning，但不真实推送；需要调整重试次数可设置 `GITHUB_PREFLIGHT_RETRIES=5`。需要机器读取时用 `node scripts/verify-external-preflight.mjs --json`，需要把元数据或远端 CI 查询失败视为失败时追加 `--strict-gh` 或 `--strict-remote-ci`。
