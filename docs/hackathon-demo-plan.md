...
4. 主持校验：刘看山控场，站内观点席提炼已有观点，反方校验席挑战证据不足，刘看山追问席判断是否愿意参与。
...
| 26 | 主持校验 | 刘看山追问席判断是否看得懂、愿意回 | 校验发言流 | Ready |
...
画面：打开首页，停在 `刘看山主持的知乎讨论组织台` 首屏和主按钮 `从热榜生成讨论方案`。
...
- 刘看山追问席：提出刘看山会继续追问的问题。
- 每个席位都有 mission、tone、证据约束，证明它们是在校验讨论方案，而不是随机聊天。

操作：

- 展示刘看山、站内观点席、反方校验席和刘看山追问席的任务分工。
...
单次生成容易直接给结论，多个校验席位可以形成制衡：刘看山控场、站内观点席补结构、反方校验席找漏洞、刘看山追问席判断是否愿意参与。最后系统把分歧沉淀成站队选项、风险提醒和下一轮追问，比单篇回答更像知乎社区里的真实讨论组织。
