...
如平台需要源码压缩包，使用 npm run package:source 生成 .cache/submission/zhihu-roundtable-source.zip 和 .cache/submission/manifest.json；manifest 会记录 HEAD commit、文件数、ZIP 大小和 sha256。提交备注或路演备忘可用 npm run evidence:submission 打印当前 commit、源码包 sha256、截图尺寸、支撑材料和最终外部验收命令；需要可直接粘贴的格式时使用 `node scripts/print-submission-evidence.mjs --markdown`，本地预览才追加 `--allow-dirty`。
```
