...
如平台需要源码压缩包，使用 npm run package:source 生成 .cache/submission/zhihu-roundtable-source.zip 和 .cache/submission/manifest.json；manifest 会记录 HEAD commit、跟踪文件数、ZIP 实际文件数、ZIP 大小和 sha256。提交备注或路演备忘可用 npm run evidence:submission 打印当前 commit、源码包文件数/ZIP 实际文件数、sha256、截图尺寸、支撑材料和最终外部验收命令；需要可直接粘贴的格式时使用 `node scripts/print-submission-evidence.mjs --markdown`，本地预览才追加 `--allow-dirty`。
```
...
对内容消费者：它把分散、情绪化、重复的热点讨论组织成可参与的问题；读者可以站队、补充经验、提出反例。
