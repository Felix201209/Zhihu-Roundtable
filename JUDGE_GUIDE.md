...
- **不是 AI 写回答**：热榜会被压成可站队、可追问、可回流的讨论。
- **刘看山负责质检**：发布前检查“能站队、反方成立、证据够用、表达不引战”。
- **评论继续往前走**：新反方、真实经验和高质量补充进入下一轮话题。
...
3. 进入 `刘看山主持校验`，确认角色为站内观点席、反方校验席、刘看山追问席，且发言绑定来源标签。
4. 进入 `发布策划与圈子帖预览`，确认有站队选项、引导评论、风险提醒、发布安全提示，且发布前有人工确认。
5. 发布后看 `评论复盘与下一轮创作`，确认真实评论或演示评论会生成下一篇内容方向。
6. 展开 `技术证据`，看接口、模型调用、节点和 readiness 证据。
